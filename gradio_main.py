...
def _generate_audio_instance(text_piece,
    model_choice,
    language,
    speaker_audio,
    prefix_audio,
    ...
    estimated_generation_duration = 30 * len(text_piece) / 400
    ...
    return (sr_out, wav_out[0]), seed

def generate_audio(source_text, *args):
    ...
    for i, text_piece in enumerate(texts):
        ...
        (sr, wav), seed = _generate_audio_instance(text_piece, *args)
    
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
             torchaudio.save(f.name, wav, sr)
             merged_audio += AudioSegment.from_wav(f.name) + AudioSegment.silent(duration=500)
             os.remove(f.name)
             
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
        merged_audio.export(f.name, format='wav')
        print("Normalizing audio...")
        os.system(f'ffmpeg -i {f.name} -af "loudnorm" {f.name.replace(".wav", "_norm.wav")}')
    
    wav_out, sample_rate = torchaudio.load(f'{f.name.replace(".wav", "_norm.wav")}')
    ...
    os.remove(f.name)
    os.remove(f'{f.name.replace(".wav", "_norm.wav")}')

    return (sample_rate, wav_out), seed
